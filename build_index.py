...
def main():
    ...
    for mp4 in sorted(VIDS.glob("*.mp4")):
        ...
        if n_gt is not None and n_gt != n_pts:
            meta += f" (GT panel: N={n_gt})"
        cards.append(
...
<header>
  ...
  <p>
    ...
    AllTracker, then lifted to 3D using ViPE depth/pose/intrinsics on the same
    Sora video. Left: source GT clip + GT 2D tracks of the query subset.
    Middle: Sora 2 video + AllTracker 2D tracks. Right: ViPE-lifted 3D
    (camera-frame, gentle azimuth rotation). Each panel uses an independent
    rainbow palette indexed by query-point order.
  </p>
</header>
<main>
{chr(10).join(cards)}
